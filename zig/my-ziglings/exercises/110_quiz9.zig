...
pub fn main() !void {
    ...
    print("^ {b:0>4} // (bitmask)\n", .{0b0101});
    PORTB ^= (1 << 2) | (1 << 0); // What's wrong here? => 0100 | 0001 = 0101 ^ 1100 = 1001
    ...
    print("  {b:0>4} // (initial state of PORTB)\n", .{PORTB});
    print("^ {b:0>4} // (bitmask)\n", .{0b0011});
    PORTB ^= (3 << 0) & (3 << 0); // What's wrong here? => 0011 & 0011 = 0011 ^ 1100 = 1111
    ...
    print("  {b:0>4} // (reset state)\n", .{PORTB});
    print("& {b:0>4} // (bitmask)\n", .{0b1110});
    PORTB &= ~@as(u4, 1 << 0); // What's missing here? ~ 0001 = 1110 & 0111 = 0110
